...
```shell
kubeadm init --kubernetes-version=1.28.2 --apiserver-advertise-address=10.0.0.12 --image-repository=kubernetes-register.xy.com/google_containers --pod-network-cidr="10.24.0.0/16" --service-cidr="10.96.0.0/12" --ignore-preflight-errors=Swap  --cri-socket=unix:///var/run/cri-dockerd.sock
```

- node节点加入

```shell
kubeadm join 10.0.0.12:6443 --token xxxxxxx --discovery-token-ca-cert-hash xxxxxxxx --cri-socket=unix:///var/run/cri-dockerd.sock
```
